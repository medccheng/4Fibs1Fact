...
namespace TileMeUpWebApi.Controllers
{
    ...
    public class ItemController : ControllerBase
    {
        ...
        [HttpPost("AddImage")]
        public async Task<ActionResult> AddItemImage(ImageModel image)
        {
            string path = _configuration.GetValue<string>("Settings:ImagesBaseFolder");               
            const int maxContentLength = 512 * 512 * 1;

            if (image.Bytes.Length > maxContentLength ||
                image == null)
                return BadRequest();

            var imgUrl = ImageHelper.WriteImage(image.Bytes, path);
